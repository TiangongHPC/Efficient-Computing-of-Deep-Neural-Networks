...
class FCOSOutputs(object):
    ...
    def predict(self, locations, box_cls_cat, reg_pred_cat):
        result = []
        for idx in range(self.num_images):
            scores = box_cls_cat[idx]
            box_regression = reg_pred_cat[idx]
            bboxes = torch.stack([
                locations[:, 0] - box_regression[:, 0],
                locations[:, 1] - box_regression[:, 1],
                locations[:, 0] + box_regression[:, 2],
                locations[:, 1] + box_regression[:, 3],
            ], dim=1)
            # print(scores.shape,bboxes.shape)
            _, cls_nums = scores.shape
            r, c = (scores > self.pre_nms_thresh).nonzero(as_tuple=True)
            scores = scores[r, c]
            bboxes = bboxes[r]
            per_box_cls = c[:, None]
            x = torch.cat((bboxes, scores[:,None], per_box_cls.float()), dim=1)
            n = x.shape[0]  # number of boxes
            if not n:  # no boxes
                # boxlist = Instances(self.image_sizes[idx])
                # boxlist.pred_boxes = Boxes(bboxes[0,None])
                # boxlist.scores = scores[0,None]
                # boxlist.pred_classes = torch.zeros([1],dtype=torch.long,device=scores.device)
                # result.append(boxlist)
                continue
            elif n > self.pre_nms_top_n:  # excess boxes
                v, k = scores.topk(self.pre_nms_top_n)
                x = x[k]
                # x = x[x[:, 4].argsort(descending=True)[:self.pre_nms_top_n]]
            bboxes, scores, labels = x[:,:4], x[:,4], x[:,5]
            ...
            if keep.shape[0] > self.fpn_post_nms_top_n:
                keep = keep[:self.fpn_post_nms_top_n]
            x = x[keep]
            boxlist = Instances(self.image_sizes[idx])
            boxlist.pred_boxes = Boxes(x[:,:4])
            boxlist.scores = x[:,4]
            boxlist.pred_classes = x[:,5].long()
            result.append(boxlist)
        return result
    ...
    def forward_for_single_feature_map(self, locations, box_cls, reg_pred, ctrness, image_sizes):
        ...
        box_regression = reg_pred.reshape(N, 4, H, W).permute(0, 2, 3, 1)
        box_regression = box_regression.reshape(N, -1, 4)
        ctrness = ctrness.reshape(N, 1, H, W).permute(0, 2, 3, 1)
