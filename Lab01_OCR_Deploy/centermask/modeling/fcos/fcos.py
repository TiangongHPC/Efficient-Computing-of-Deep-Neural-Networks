...
class FCOS(nn.Module):
    ...
    def forward(self, images, features, gt_instances):
        ...
        logits_pred, reg_pred, ctrness_pred, bbox_towers, logits_pred_cat, reg_pred_cat = self.fcos_head(features)
        ...
        proposals = outputs.predict(locations_cat, logits_pred_cat, reg_pred_cat)
...
class FCOSHead(nn.Module):
    ...
    def forward(self, x):
        ...
        logits_reshape_list = []
        bbox_reg_reshape_list = []
        ctrness_reshape_list = []
        ...
        for l, feature in enumerate(x):
            ...
            logits_t = self.cls_logits(cls_tower)
            logits.append(logits_t)
            ctrness_t = self.ctrness(bbox_tower)
            ctrness.append(ctrness_t)
            reg = self.bbox_pred(bbox_tower)
            if self.scales is not None:
                reg = self.scales[l](reg)
            # Note that we use relu, as in the improved FCOS, instead of exp.
            reg_t = F.relu(reg)
            bbox_reg.append(reg_t)

            N, C, H, W = logits_t.shape
            logits_reshape = logits_t.reshape(N, C, H, W).permute(0, 2, 3, 1).reshape(N, -1, C).sigmoid()
            bbox_reg_reshape = reg_t.reshape(N, 4, H, W).permute(0, 2, 3, 1).reshape(N, -1, 4)
            ctrness_reshape = ctrness_t.reshape(N, 1, H, W).permute(0, 2, 3, 1).reshape(N, -1).sigmoid()
            logits_reshape_list.append(logits_reshape)
            bbox_reg_reshape_list.append(bbox_reg_reshape*self.fpn_strides[l])
            ctrness_reshape_list.append(ctrness_reshape)
        logits_reshape_all = torch.cat(logits_reshape_list, dim=1)
        bbox_reg_reshape_all = torch.cat(bbox_reg_reshape_list, dim=1)
        ctrness_reshape_all = torch.cat(ctrness_reshape_list, dim=1)
        logits_reshape_all = logits_reshape_all * ctrness_reshape_all[:, :, None]
        logits_reshape_all = torch.sqrt(logits_reshape_all)
        return logits, bbox_reg, ctrness, bbox_towers, logits_reshape_all, bbox_reg_reshape_all
